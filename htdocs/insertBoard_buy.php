<?php  
  ...
  if($imageCheck){
    $result = mysqli_query($con,"
      INSERT INTO talent_buy_posting (
        id, title, dayResult, gender, writeDate, type, 
        startHour, endHour, category, talent, contents, filePath
      ) 
     VALUES (
      '$id', '$title', '$dayResult', (SELECT gender FROM userjoin WHERE userjoin.id = '$id'),
      '$writeDate', '$type', '$startHour', '$endHour', '$category', '$talent', '$contents', '$file_path')
    ");
  }else{
    $result = mysqli_query($con,"
      INSERT INTO talent_buy_posting (
     ...
     VALUES (
      '$id', '$title', '$dayResult', (SELECT gender FROM userjoin WHERE userjoin.id = '$id'),
      '$writeDate', '$type', '$startHour', '$endHour', '$category', '$talent', '$contents')
    ");
  }
