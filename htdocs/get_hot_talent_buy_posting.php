<?php
	...
	$res = mysqli_query($con, "
		SELECT 
			...
			talent_buy_posting.filePath, avg(review_buy.score),
			(SELECT count(*) 
			FROM review_buy 
			WHERE review_buy.num = talent_buy_posting.boardNumber) 
		FROM talent_buy_posting 
		INNER JOIN review_buy ON talent_buy_posting.boardNumber = review_buy.num 
		GROUP BY talent_buy_posting.boardNumber 
		ORDER BY avg(review_buy.score) DESC LIMIT 5
	");
